...
from .hypopt_track_segmentation_unet_AnDi import hypoptUNet
...
def make_preds(model, X_to_eval, y_to_eval, min_max_len=601, device='cpu',
               ...
               use_temperature_scaling=False, temperature=7.0,):
    tmp_dict = {}
    ...
    if not use_temperature_scaling:
        ...
        masked_pred, masked_score, masked_y = model.predict(test_loader, temperature=temperature)
...
def load_UnetModels_directlyANDI(best_model_path = 'Unet_results/mlruns/', device='cpu', dim=2):
...
def find_models_for_from_path(path, modelname='_UNETmodel'):
    # load from path
    files = sorted(glob(path+'/*/*'+modelname+'.torch', recursive=True))
    return files
...
class CustomDataset(Dataset):
    def __init__(self, X, Y1, Y2, Y3, Y4, X_padtoken=0, y_padtoken=10, device='cpu',
                 min_max_length=0):
        maxlens = np.max([len(x) for x in X])
        maxlens = min_max_length if maxlens < min_max_length else maxlens
        X = [torch.tensor(x, device=device) for x in X]
        Y1 = [torch.tensor(y, device=device) for y in Y1]
        Y2 = [torch.tensor(y, device=device) for y in Y2]
        Y3 = [torch.tensor(y, device=device) for y in Y3]
        Y4 = [torch.tensor(y, device=device) for y in Y4]
        ...
        self.y1 = [nn.ConstantPad1d((maxlens-len(y), 0), y_padtoken)(
                            y.permute(*torch.arange(y.ndim - 1, -1, -1))).float() for y in Y1]
        self.y2 = [nn.ConstantPad1d((maxlens-len(y), 0), y_padtoken)(
                            y.permute(*torch.arange(y.ndim - 1, -1, -1))).float() for y in Y2]
        self.y3 = [nn.ConstantPad1d((maxlens-len(y), 0), y_padtoken)(
                            y.permute(*torch.arange(y.ndim - 1, -1, -1))).float() for y in Y3]
        self.y4 = [nn.ConstantPad1d((maxlens-len(y), 0), y_padtoken)(
                            y.permute(*torch.arange(y.ndim - 1, -1, -1))).float() for y in Y4]
    def __len__(self):
        return len(self.X)
...
def create_temporalfingerprint(tracks, ensemble_pred, fp_datapath, hmm_filename, dim, dt,
                            selected_features=[0], difftype='Normal'):
    '''
