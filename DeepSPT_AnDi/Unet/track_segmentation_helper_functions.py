...
from .hypopt_track_segmentation_unet import hypoptUNet
from .track_segmentation_unet import UNet
...
class TestSetData(Dataset):
    def __init__(self, X, Y, X_padtoken=0, y_padtoken=10, maxlens=0, device='cpu'):
        ...
        self.y = [nn.ConstantPad1d((maxlens-len(y), 0), y_padtoken)(y.T).float() for y in Y]
    def __len__(self):
        return len(self.X)
...
def remove_short_traces3D(df, threshold=30):
    ...
    return df[df['id'].isin(keep_particles)]
...
def cme_tracks_to_pandas(mat_path, project_name):
    ...
    """
    COLUMNS = "A", "AS", "ASM", "x", "y", "z", "c", 'sigma_r', 't', 'f', 'catIdx', 'nSeg'
    ...
    try:
        ...
        for i in range(n_tracks):
            # # Extract columns
            try:
                A, AS, ASM, x, y, z, c, sig, t_in_seconds, frames, catIdx, nSeg = [index(m=m, track_id=i, column=c) for c in COLUMNS]
            except:
                COLUMNS = "A", "x", "y", "z", "c", 'sigma_r', 't', 'f', 'catIdx', 'nSeg'
                A, x, y, z, c, sig, t_in_seconds, frames, catIdx, nSeg = [index(m=m, track_id=i, column=c) for c in COLUMNS]
...
def load_or_pred_for_simdata(X_to_eval, y_to_eval, globals, savepath, savename, 
    ...
    if os.path.exists(savepath+savename):
        ...
        results_dict = {}
        for modelname in best_models_sorted:
            model = load_UnetModels(modelname)
    ...
    with torch.no_grad():
        ...
        for idx, xb in enumerate(val_loader):
            ...
            for i in range(len(y)):
                ...
                masked_preds.append(masked_pred.numpy())
                ...
                for modelname in best_models_sorted:
                    if load_old:
                        ...
                        if use_mlflow:
                            ...
                            model = load_UnetModels_directly(modelname, dir=modelpath, device=device)
...
def compare_pred2sim_diffusion(trace_list, y_list, pred_list, savename=''):
    color_dict = {'0':'blue', '1':'red', '2':'green', '3':'darkorange'}
    for i, track in enumerate(trace_list):
        fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2,figsize=(13,5))
        x,y = track[:,0], track[:,1]
        c_sim = [colors.to_rgba(color_dict[str(int(label))]) for label in y_list[i]]
        c_pred = [colors.to_rgba(color_dict[str(label)]) for label in pred_list[i]]
        ...
        acc = np.mean(np.hstack(y_list[i]) == np.hstack(pred_list[i]))
        ...
        ax1.set_title('Simulated trace length: {}'.format(len(y_list[i])))
...
def load_3D_data(file, SEARCH_PATTERN, OUTPUT_NAME,
    ...
    # Turn matlab df into useful numpy arrays
    df = matlabloader(name=file, input=SEARCH_PATTERN, output=OUTPUT_NAME, dont_use=dont_use)
    df = remove_short_traces3D(df, threshold=min_trace_length) 
    df = remove_dim_traces3D(df, threshold=min_brightness)
    df = remove_overlybright_traces3D(df, threshold=max_brightness)
...
def handle_compound_tracks(compound_idx, tracks, 
    ...
    for ci in compound_idx:
        ...
        tp_list = []
        idx_list = [] 
        filler0_fission1_concat2_fusion3 = []
        changepoints_all = []
        for i, cp in enumerate(cps[:-1]):
            if i == 0:
                tp_list.append(timepoints[ci][:cps[i+1]])
                idx = all_idx[:cps[i+1]]
                idx_list.append(idx)
                ...
                # +1 because dont want the nan
                tp = timepoints[ci][cp+1:cps[i+1]]
                idx = all_idx[cp+1:cps[i+1]]
                together_idx = ~np.in1d(tp_list[i-1], tp)
                if together_idx[-1] == False: # fission
                    filler0_fission1_concat2_fusion3.append(1)
                    tp = np.hstack([tp_list[i-1][together_idx], tp])
                    idx = np.hstack([idx_list[i-1][together_idx], idx])
                elif np.mean(together_idx.astype(int))==1: # concat
                    filler0_fission1_concat2_fusion3.append(2)
                    tp = np.hstack([tp_list[i-1], tp])
                    idx = np.hstack([idx_list[i-1], idx])
                else: # fusion
                    filler0_fission1_concat2_fusion3.append(3)
                    # +1 because dont want to have fuse timepoint twice
                    fuse_idx = np.array(range(len(idx_list[i-1])))[~together_idx][-1]+1
                    end_fuse = np.array(range(len(idx_list[i-1])))[-1]
                    tp = np.hstack([tp, tp_list[i-1][fuse_idx:end_fuse+1]])
                    idx = np.hstack([idx, idx_list[i-1][fuse_idx:end_fuse+1]])
                ...
                if len(find_timepointchange(tp)) != 2:
                    print('error')

                assert len(find_timepointchange(tp)) == 2, f"{find_timepointchange(tp)} {tp}"
                ...
                if len(tracks[ci][idx])>=min_len_threshold and amplitudes[ci][idx][0,0]>=min_bright:
                    ...
                    amplitudes_bg.append(amplitudes_bg[ci][idx])
                    track_ids.append(track_ids[ci]+'_c'+str(i))
        fission0_concat1_fusion2_all.append([0]+filler0_fission1_concat2_fusion3+[0])
        changepoints_all.append(cps)
